            ...
            <li class="nav_down">
                ...
                <ul id="nav_down" class="t_center">
                    <c:forEach items="${sessionScope.Subject}" var="subject">
                        <li>
                            <a target="_blank"
                               href="${pageContext.request.contextPath}/toCourse?subjectId=${subject.id}">${subject.subjectName}</a>
                        </li>
                    </c:forEach>
                </ul>
            </li>
...
<div id="app">
    <!--banner图-->
    <div class="banner"
         style="background-image: url(${pageContext.request.contextPath}/img/banner-${sessionScope.subjectList.id}.jpg)"></div>

    <!--面包屑导航-->
    <div class="container mian-nav">${sessionScope.subjectList.subjectName}</div>

    <div class="classify">
        <div class="container">
            <!--章-->
            <c:forEach items="${sessionScope.subjectList.courses}" var="course">
                <div class="section">
...
<script type="text/javascript">
    ...
    function getVideo(videoId) {
        ...
        if ((null != "${sessionScope.userAccount}" && "${sessionScope.userAccount}" != "") || ($("#isLogin").val() == 1)) {
            //如果登录
            location.href = "${pageContext.request.contextPath}/showVideo?videoId=" + videoId + "&subjectName=" + '${sessionScope.subjectList.subjectName}';
        } else {
            //如果不登录，弹登录框
